...
export const siteSchema = z.object({
  ...
  // Electrical info
  utilityCompany: z.string().trim().min(1, "Concessionária inválida"),
  uc: z.string().trim().min(1, "UC inválida"),
  accountHolder: z.string().trim().min(1, "Titular inválido"),
  serviceType: z.object({
    class: z.enum(["residential", "commercial", "industrial"]).default("residential"),
    connection: z.enum(["single-phase", "two-phase", "three-phase"]).default("single-phase"),
